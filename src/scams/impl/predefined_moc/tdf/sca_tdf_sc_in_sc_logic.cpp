...
/*****************************************************************************
 ...
 SVN Id            :  $Id: sca_tdf_sc_in_sc_logic.cpp 2320 2023-06-08 12:47:38Z pehrlich $
...
void sca_in<sc_dt::sc_logic>::initialize_port()
{
	if(this->buffer!=NULL)
	{
	...
	this->buffer=new sc_dt::sc_logic[this->buffer_size];
	...
	const sc_dt::sc_logic& init_val=(*this)->read();
...
void sca_in<sc_dt::sc_logic>::reinitialize_port()
{
	std::uint64_t new_buffer_size=this->get_delay_internal() + this->get_rate_internal() *
			this->sca_core::sca_implementation::sca_port_base::get_parent_module()->get_calls_per_period();


	sc_dt::sc_logic* new_buffer=NULL;
	if(this->buffer_size!=new_buffer_size)
	{
		new_buffer=new sc_dt::sc_logic[new_buffer_size];
	}
	else
	{
...
inline const sc_dt::sc_logic& sca_in<sc_dt::sc_logic>::read_buffer(unsigned long n)
{
...
inline void sca_in<sc_dt::sc_logic>::read_from_sc()
{
...
void sca_in<sc_dt::sc_logic>::initialize(const sc_dt::sc_logic& value, unsigned long sample_id)
{
	...
	if (sample_id >= this->get_delay_internal())
	{
		...
		str << "Sample id (" << sample_id << ") is greater than delay ("
				<< sca_in<sc_dt::sc_logic>::get_delay() << " while initializing port: " << sca_in<sc_dt::sc_logic>::name();
...
void sca_in<sc_dt::sc_logic>::trace(long id, sca_util::sca_implementation::sca_trace_file_base& tf)
{
